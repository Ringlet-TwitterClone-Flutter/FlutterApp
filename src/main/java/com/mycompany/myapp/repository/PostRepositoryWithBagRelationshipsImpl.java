...
public class PostRepositoryWithBagRelationshipsImpl implements PostRepositoryWithBagRelationships {
    ...
    public Optional<Post> fetchBagRelationships(Optional<Post> post) {
        return post.map(this::fetchHashtags);
    }
    ...
    public List<Post> fetchBagRelationships(List<Post> posts) {
        return Optional.of(posts).map(this::fetchHashtags).orElse(Collections.emptyList());
    }

    Post fetchHashtags(Post result) {
        return entityManager
            .createQuery("select post from Post post left join fetch post.hashtags where post is :post", Post.class)
            .setParameter("post", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Post> fetchHashtags(List<Post> posts) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, posts.size()).forEach(index -> order.put(posts.get(index).getId(), index));
        List<Post> result = entityManager
            .createQuery("select distinct post from Post post left join fetch post.hashtags where post in :posts", Post.class)
